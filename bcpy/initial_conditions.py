...
class InitialConditions:
  ...
  def evaluate_gaussians(self,A,a,b,c,x0:np.ndarray,z0:np.ndarray):
    ...
    wz_num = np.zeros(shape=(nwz,self.domain.num_coor[0].shape[0],self.domain.num_coor[1].shape[0]), dtype=np.float64)
  ...
  def plot_gaussian(self,wz_num):
    ...
    nwz = wz_num.shape[0]
    field = np.zeros(shape=(self.domain.num_coor[0].shape[0],self.domain.num_coor[1].shape[0]), dtype=np.float64) 
    for n in range(nwz):
      field += wz_num[n,:,:]
