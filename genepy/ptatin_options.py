...
class Model:
  ...
  .. py:class:: Model(model_ics:InitialConditions, model_regions:ModelRegions, model_bcs:ModelBCs, model_name:str="model_GENE3D", **kwargs)
    ...
    **Keyword arguments (optional)**
    ...
    :param int mpi_ranks: 
      Number of MPI ranks, it is used to define the solver options. 
      For now, there are 4 possible values: ``1``, ``64``, ``512`` and ``1024``.
      Any value other value will set the solver options for ``1`` MPI rank.
      These options were defined for some meshes but may require some modification depending
      on users needs and computational resources. Default is ``1``. 
  ...
  def __init__(self, model_ics:InitialConditions, 
    ...
    if ranks == 1: self.solvers_1MPIrank()
    elif ranks == 64: self.solvers_64MPIranks()
    elif ranks == 512: self.solvers_512MPIranks()
    elif ranks == 1024: self.solvers_1024MPIranks()
  ...
  def solvers_512MPIranks(self) -> None:
    self.options += "##############################################\n"
    self.options += "########### SOLVERS 512 MPI ranks ############\n"
    self.options += "##############################################\n"
    
    self.options += "########### Stokes ###########\n"
    self.options += "# 128 x 48 x 128\n"
    self.options += "# 64  x 24 x  64\n"
    self.options += "# 32  x 12 x  32\n"
    self.options += "# 16  x 12 x  16\n"
    self.options += "# 2 Assembled galerkin \n"
    self.options += "# 0 Assembled rediscretized \n"
    self.options += "# 1 Matrix-free \n"
    self.options += "-A11_operator_type 2,0,1,1\n"
    self.options += "-dau_nlevels 4\n"
    self.options += "-stk_velocity_dm_mat_type aij\n"
    self.options += "-a11_op avx\n"
    self.options += "-stk_velocity_da_refine_hierarchy_x 2,2,2\n"
    self.options += "-stk_velocity_da_refine_hierarchy_y 2,2,1\n"
    self.options += "-stk_velocity_da_refine_hierarchy_z 2,2,2\n"
    self.options += "###### SNES ######\n"
    self.options += "-newton_its 0\n"
    self.options += "-snes_atol 1e-6\n"
    self.options += "-snes_max_it 5\n"
    ...
    self.options += "\t\t###### Coarse Grid ######\n"
    self.options += "\t\t## 16 x 12 x 16\n"
    self.options += "\t\t# 8  x 6  x 8 \n"
    self.options += "\t\t# 4 x 3 x 4\n"
    self.options += "\t\t-fieldsplit_u_mg_coarse_pc_type telescope\n"
    self.options += "\t\t-fieldsplit_u_mg_coarse_ksp_type preonly\n"
    self.options += "\t\t-fieldsplit_u_mg_coarse_pc_telescope_reduction_factor 8 \n"
    self.options += "\t\t# From 512  MPI ranks, new there are 512/8 = 64 MPI ranks\n"
    self.options += "\t\t-fieldsplit_u_mg_coarse_telescope_pc_type mg\n"
    self.options += "\t\t-fieldsplit_u_mg_coarse_telescope_ksp_type preonly\n"
    self.options += "\t\t-fieldsplit_u_mg_coarse_telescope_pc_mg_levels 2\n"
    self.options += "\t\t-fieldsplit_u_mg_coarse_telescope_pc_mg_galerkin\n"
    self.options += "\t\t-fieldsplit_u_mg_coarse_telescope_mg_levels_ksp_type fgmres\n"
    self.options += "\t\t-fieldsplit_u_mg_coarse_telescope_mg_levels_pc_type jacobi\n"
    self.options += "\t\t-fieldsplit_u_mg_coarse_telescope_mg_levels_ksp_max_it 10\n"
    ...
    self.options += "\t-energyfv_mg_levels_pc_type jacobi\n"
    self.options += "\t-energyfv_mg_coarse_pc_type telescope\n"
    self.options += "\t-energyfv_mg_coarse_pc_telescope_reduction_factor 8 \n"
    self.options += "\t# MPI ranks:  512/8  = 64\n"
    self.options += "\t\t-energyfv_mg_coarse_telescope_pc_type gamg\n"
    self.options += "\t\t-energyfv_mg_coarse_telescope_mg_levels_ksp_type fgmres\n"
    self.options += "\t\t-energyfv_mg_coarse_telescope_mg_levels_pc_type jacobi\n"
    self.options += "\t\t-energyfv_mg_coarse_telescope_mg_levels_ksp_max_it 8\n"
  ...
  def solvers_64MPIranks(self) -> None:
    self.options += "##############################################\n"
    self.options += "############ SOLVERS 64 MPI ranks ############\n"
    self.options += "##############################################\n"

    self.options += "########### Stokes ###########\n"
    self.options += "# 64 x 24 x 64\n"
    self.options += "# 32 x 12 x 32\n"
    self.options += "# 16 x 6  x 16\n"
    self.options += "# 8  x 6  x 8 \n"
    self.options += "# 2 Assembled galerkin \n"
    self.options += "# 0 Assembled rediscretized \n"
    self.options += "# 1 Matrix-free \n"
    ...
    self.options += "-a11_op avx\n"
    self.options += "-stk_velocity_da_refine_hierarchy_x 2,2,2\n"
    self.options += "-stk_velocity_da_refine_hierarchy_y 2,2,2\n"
    self.options += "-stk_velocity_da_refine_hierarchy_z 2,2,1\n"
    self.options += "###### SNES ######\n"
    self.options += "-newton_its 0\n"
    self.options += "-snes_atol 1e-6\n"
    self.options += "-snes_max_it 5\n"
    ...
    self.options += "\t-fieldsplit_u_mg_levels_ksp_norm_type NONE\n"
    self.options += "\t\t###### Coarse Grid ######\n"
    self.options += "\t\t# 6 x 6 x 6\n"
    self.options += "\t\t-fieldsplit_u_mg_coarse_pc_type telescope\n"
    self.options += "\t\t-fieldsplit_u_mg_coarse_ksp_type preonly\n"
    self.options += "\t\t-fieldsplit_u_mg_coarse_pc_telescope_reduction_factor 64\n"
    self.options += "\t\t# From 24 MPI ranks, new there are 24/24 = 1  MPI ranks\n"
    self.options += "\t\t# 6 x 6 x 6\n"
    self.options += "\t\t-fieldsplit_u_mg_coarse_telescope_pc_type cholesky\n"
    ...
    self.options += "\t-energyfv_mg_levels_pc_type jacobi\n"
    self.options += "\t-energyfv_mg_coarse_pc_type telescope\n"
    self.options += "\t-energyfv_mg_coarse_pc_telescope_reduction_factor 64\n"
    self.options += "\t# MPI ranks: 24/24 = 1\n"
    self.options += "\t\t-energyfv_mg_coarse_telescope_pc_type lu\n"
    self.options += "-fvpp_ksp_monitor\n"
    self.options += "-fvpp_ksp_rtol 1.0e-10\n"
    self.options += "-fvpp_ksp_type cg\n"
    self.options += "-fvpp_mg_coarse_pc_type redundant\n"
